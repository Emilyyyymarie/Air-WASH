...
#Also store to csv
write.csv(PMstovedataframetotal,file.path(currentresultspath,paste("PMstovedata",country,".csv",sep="")),row.names=FALSE)
...
#this is the air and water boiling plot

jpeg(file=file.path(currentresultspath,paste("adultchildfinalplot_v3",country,".jpg",sep="")), res=600, width=20000, height=10000, pointsize=10,
     type="windows", antialias="cleartype")
newplotnewboilingdiffhello<-ggplot(data=Boilingbardataframetotal, aes(x=boilingbargrouprisk, y=boilingbargrouptotal, fill=Category))+
  geom_bar(stat="identity", position=position_dodge(),fill="white")+theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.background = element_blank(), axis.line = element_line(colour = "black"),text = element_text(size = 20),axis.text=element_text(size=20),legend.key=element_blank(),plot.title = element_text(hjust = 0.5))+ggtitle("") +
...
write.csv(Boilingbardataframetotal,file.path(currentresultspath, paste("alldata_mainplot_total",country,".csv",sep="")),row.names=FALSE)
write.csv(Boilingbardataframechild,file.path(currentresultspath, paste("alldata_mainplot_child",country,".csv",sep="")),row.names=FALSE)
write.csv(Boilingbardataframe,file.path(currentresultspath, paste("alldata_mainplot_adult",country,".csv",sep="")),row.names=FALSE)
...
newplotnewnew<-ggplot(data=PMstovedataframetotal,aes(x=Stovenames,y=PMlevelstotal,fill=Stove_use))+ 
  ...
  ylab("Total DALYs for different stove (10,000 ppl)")+xlab("Stove Type")+geom_errorbar(aes(ymin=PMlevelstotal-PMsdtotal, ymax=PMlevelstotal+PMsdtotal), width=.2,position=position_dodge(.9))+geom_bar(data=PMstovedataframe,aes(x=Stovenames,y=PMlevels,fill=Stove_use),stat="identity",position=position_dodge(),alpha=0.99,colour="black")+geom_bar(data=PMstovedataframetotal, aes(x=Stovenames, y=PMlevelstotal, fill=Stove_use),stat="identity", position=position_dodge(),alpha=0.3,colour="black")+theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.background = element_blank(), axis.line = element_line(colour = "black"),text = element_text(size = 15),axis.text=element_text(size=15),legend.key=element_blank(),plot.title = element_text(hjust = 0.5))
write.csv(PMstovedataframetotal,file.path(currentresultspath, paste("PMstovedataframetotal_2",country,".csv",sep="")),row.names=FALSE)
write.csv(PMstovedataframe,file.path(currentresultspath, paste("PMstovedataframe_adult",country,".csv",sep="")),row.names=FALSE)
write.csv(PMstovedataframechild,file.path(currentresultspath,paste("PMstovedataframe_child",country,".csv",sep="")),row.names=FALSE)
print(newplotnewnew)
